...
def _email_inputs(fn_meta, fn_name, latest_error, min_exec_tm,
    ...
    subject = f'PyNotify: {fn_name} Executed {status} ({min_exec_tm})'
    ...
    <html>
      ...
      <body>
        <p><b>Executed:</b> {code}...<br><br>
           ...
           {err_tb}
           --<br>
           <b>See The Docs:</b> <a href="http://www.python.org">PyNotify</a> 
        </p>
      </body>
    </html>
    """
...
def format_msg(fn_meta, fn_name, min_exec_tm):
...
def _exec_func(function, email, timeit, logger, printf, notes, error, host, 
    ...
    _log_msg(logger, printf, format_msg(fn_meta, fn_name, min_exec_tm))
